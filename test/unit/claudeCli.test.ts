...
describe("ClaudeCliProvider", () => {
  ...
  it("calls claude -p with haiku, text output and no MCP, from the home dir", async () => {
    ...
    expect(cap.args).toEqual(
      expect.arrayContaining(["-p", "--model", "haiku", "--output-format", "text", "--strict-mcp-config"]),
    );
    expect(cap.args).not.toContain("--max-turns");
